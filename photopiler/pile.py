...
from .options import SCALE_FRACTION, PileOptions
...
def style_photos(photos, options: PileOptions) -> list[Image.Image]:
    """Apply the per-photo effects, turning each photo into a finished print.
    ...
    another option's random choices.
    """
    ...
    for i, photo in enumerate(photos):
        # Tone before border, so borders stay paper-white on monochrome prints;
        # aging last, so the borders yellow and pick up grain along with the photo.
        photo = apply_tone(photo, resolve_tone(options.tone, tone_rng))
        border = resolve_border(options.border, border_rng)
        left, top, _, _ = border_widths(photo.size, border)
        photo_box = (left, top, left + photo.width, top + photo.height)
        photo = add_border(photo, border)
        noise_rng = np.random.default_rng([options.seed & 0xFFFFFFFF, i])
        photo = age_photo(photo, resolve_age(options.age, age_rng), noise_rng, photo_box)
        prints.append(photo)
    return prints
...
def render_pile(photos, options: PileOptions) -> Image.Image:
    ...
    prints = style_photos(photos, options)
    print_sizes = [p.size for p in prints]
    side = canvas_side(print_sizes, options.scale)

    # If the size cap kicked in, shrink prints to keep their proportion of the canvas.
    shrink = min(1.0, side / uncapped_side(print_sizes, options.scale))
    canvas = table.surface(side, options.tilt, unit=shrink)

    for placement in layout:
        photo = prints[placement.index]
        if shrink < 1:
            photo = photo.resize(
        ...
        cx, cy = image_center(placement, rotated.size, side, options.tilt)

        layer, pad = with_shadow(rotated)
        if options.tilt > 0:
            # Lay the print on the table where the camera sees this spot, keeping
            # its real size, and project it into the image.
            tx, ty = camera.table_point(side, options.tilt, cx, cy)
            top_left = (tx - layer.width / 2, ty - layer.height / 2)
            projected = camera.project(layer, top_left, side, options.tilt)
            if projected:
                layer, position = projected
                canvas.paste(layer, position, layer)
        else:
            top_left = (round(cx - rotated.width / 2) - pad, round(cy - rotated.height / 2) - pad)
            canvas.paste(layer, top_left, layer)
